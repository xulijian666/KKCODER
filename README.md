...
通过 Tauri v2 的高性能跨语言渲染、轻量级的 SQLite 本地持久化、以及高度定制的 xterm.js 虚拟终端，KKCoder 创造了一个集**多会话项目树管理、无痕临时调试、多主题自适应、系统级通知提示**于一体的高端开发心流保护区。
...
### 🗑️ 2. 会话软删除与自动清理 (Soft Delete & Auto Cleanup)
* **右键软删除**：左侧会话支持右键删除，删除后记录即刻从列表隐藏，不占界面。

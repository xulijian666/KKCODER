      ...
      <div aria-busy={loading || showSummarySkeleton || showProviderSkeleton}>
        ...
        <div className="flex items-center justify-between gap-3 mb-8">
          <div
            ref={tablistRef}
            role="tablist"
            aria-label={copy("usage.overview.tablist_aria")}
            onKeyDown={handleTablistKeyDown}
            className="flex items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden p-1 rounded-xl border border-[var(--border-color)] bg-[var(--bg-active-item)]"
          >
            {tabs.map((p) => {
              const isActive = period === p.key;
              const tabClass = `shrink-0 whitespace-nowrap text-xs font-mono px-3 py-1.5 rounded-lg transition-all cursor-pointer font-semibold select-none ${
                isActive
                  ...
                  >
                    ...
                    <Popover.Portal>
                      <Popover.Positioner sideOffset={8} side="bottom" align="start" className="!z-[9999]">
                        <Popover.Popup className="bg-[var(--bg-sidebar)] border border-[var(--border-color)] rounded-xl shadow-2xl p-0">
                          <DateRangePopover
                            from={customFrom}
                            to={customTo}
          ...
          <div className="flex items-center gap-2 shrink-0">
            {deviceOptions.length > 1 ? (
              <Select
                ...
                matchTriggerWidth
                className="h-8 px-3 text-xs font-medium rounded-lg border border-[var(--border-color)] bg-[var(--bg-active-item)] text-[var(--text-primary)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors duration-200"
              />
            ) : null}
            {onOpenShare ? (
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 h-8 px-3 text-xs font-medium rounded-lg border border-[var(--border-color)] bg-[var(--bg-active-item)] text-[var(--text-primary)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors duration-200 cursor-pointer"
              >
        ...
        <div className="text-center mb-9 select-none">
          <div className="text-xs font-mono font-semibold text-[var(--text-secondary)] uppercase tracking-widest mb-3">{summaryLabel}</div>
          <div className="relative text-5xl sm:text-6xl md:text-7xl font-black font-mono text-[var(--text-primary)] tracking-tight tabular-nums my-2">
            <div className={showSummarySkeleton ? "invisible" : undefined}>
              {onToggleSummaryFormat ? (
                <button
                  ...
                  className="cursor-pointer rounded-lg leading-none transition-opacity hover:opacity-80 focus:outline-none"
                >
                  {summaryContent}
                </button>
              ) : (
            ...
            <div className="flex items-center justify-center gap-1.5 mt-4 mb-8">
              {onCostInfo ? (
                <button
                  type="button"
                  onClick={onCostInfo}
                  className="inline-flex items-center gap-1.5 px-2 py-1 text-base font-bold font-mono text-[var(--color-primary)] hover:opacity-80 transition-opacity cursor-pointer select-none"
                  aria-label={copy("usage.overview.cost_breakdown_aria")}
                >
                  <span>{summaryCostValue}</span>
                  <Info size={14} strokeWidth={2} className="opacity-60 hover:opacity-100" />
                </button>
              ) : (
                <span className="text-base font-bold font-mono text-[var(--color-primary)] select-none">{summaryCostValue}</span>
              )}
            </div>
          )}
        </div>
        ...
        {providers.length > 0 && (
          <div className="mt-6 mb-2">
            {/* Distribution Bar — CC-GUI 经典多色供应商横向分段进度条 (加粗至 12px 饱满质感) */}
            <div
              role="img"
              ...
              className="h-[12px] w-full bg-white/10 rounded-full overflow-hidden flex shadow-inner my-7 relative border border-[var(--border-color)]"
            >
              {providers.map((provider, idx) => {
                ...
                const pct = getProviderPercentValue(provider);
                return (
                  <motion.div
                    key={`${provider.label}-${period}-${customFrom || ""}-${customTo || ""}`}
                    initial={{ width: 0 }}
                    animate={{ width: `${pct}%` }}
                    transition={{
                      duration: 0.75,
            ...
            {/* Provider Cards — 供应商卡片与上方长条保持 24px 呼吸间距 */}
            <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3.5 mt-7">
              <button
                type="button"
        ...
        return (
          <div key={model.id || model.name} data-model-rank-row>
            <div className="grid grid-cols-[minmax(0,1fr)_minmax(8rem,max-content)_minmax(5.5rem,max-content)_4rem] items-baseline gap-x-3 mb-1.5">
              <span
                className="col-start-1 row-start-1 min-w-0 text-[13px] font-semibold font-mono text-[var(--text-primary)] truncate tracking-tight"
                title={model.name}
              >
                {model.name}
              </span>
              <span
                ...
                className="col-start-2 row-start-1 text-right whitespace-nowrap text-sm text-[var(--text-secondary)] font-mono tabular-nums"
              >
                {tokensLabel}
              </span>
              <span className="col-start-3 row-start-1 text-right whitespace-nowrap text-sm text-[var(--text-secondary)] font-mono tabular-nums">
                {costLabel}
              </span>
              <span className="col-start-4 row-start-1 text-right whitespace-nowrap text-sm font-bold font-mono text-[var(--text-primary)] tabular-nums">
                {model.share}%
              </span>
            </div>
            <div

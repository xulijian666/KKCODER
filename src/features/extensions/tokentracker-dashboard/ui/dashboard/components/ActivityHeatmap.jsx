  ...
  return (
    <div
      ...
      className={
        embedded
          ? "relative"
          : "relative rounded-2xl border border-[var(--border-color)] bg-[var(--bg-sidebar)] p-6 shadow-sm"
      }
    >
      {/* Header */}
      {!embedded && (
      <div className="flex items-baseline justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono text-[var(--text-secondary)] font-medium bg-white/5 border border-white/5 px-2.5 py-0.5 rounded-md select-none">
            {timeZoneShortLabel || copy("heatmap.legend.utc")}
          </span>
        </div>
      </div>
      )}
      ...
      {isModalOpen && (
        <div 
          ...
          className={`fixed inset-0 z-[600] flex items-center justify-center p-3 md:p-6 backdrop-blur-md bg-black/60 ${isClosing ? "animate-tt-fade-out" : "animate-tt-fade-in"}`}
        >
          {/* Modal Container */}
          <div className={`relative w-full max-w-6xl h-[88vh] backdrop-blur-2xl bg-[var(--bg-main)] border border-[var(--border-color)] shadow-[0_24px_80px_rgba(0,0,0,0.8),0_0_0_1px_rgba(255,255,255,0.1)] rounded-2xl flex flex-col md:flex-row overflow-hidden ${isClosing ? "animate-tt-modal-exit" : "animate-tt-modal"}`}>
            
            {/* Close Button */}
            <button
              type="button"
              onClick={handleCloseModal}
              className="absolute top-5 right-5 z-50 p-2 rounded-full border border-[var(--border-color)] bg-[var(--bg-active-item)] text-[var(--text-secondary)] hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] hover:rotate-90 hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer shadow-md"
            >
              <X size={16} />
            </button>

            {/* Left Side: Stats and Controls */}
            <div className="w-full md:w-[380px] border-b md:border-b-0 md:border-r border-[var(--border-color)] p-8 flex flex-col gap-6 overflow-y-auto backdrop-blur-md bg-[var(--bg-sidebar)]">
      ...
      {hoveredCell && !isModalOpen && typeof document !== "undefined" && createPortal(
        <div
          className="dark tt-dashboard fixed z-[99999] w-0 h-0 transition-all duration-100 ease-out pointer-events-none"
          style={{
            position: "fixed",
        ...
        >
          {/* Tooltip 玻璃外框（对齐图 4 工匠美学，宽裕呼吸感） */}
          <div
            className={`absolute left-0 ${tooltipPos.flipY ? "top-[12px]" : "bottom-[12px]"} backdrop-blur-2xl bg-[var(--bg-sidebar)] border border-[var(--border-color)] shadow-[0_20px_50px_rgba(0,0,0,0.8),0_0_0_1px_rgba(255,255,255,0.08)] rounded-2xl flex flex-col gap-3.5 animate-in fade-in zoom-in-95 duration-100`}
            style={{
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-2.5">
              ...
              {(() => {
                ...
                return (
                  <span 
                    className="text-[10.5px] px-2.5 py-0.5 rounded-full font-bold font-mono"
                    style={{
                      backgroundColor: "color-mix(in srgb, var(--color-primary) 14%, var(--bg-main))",
            ...
            <div className="flex flex-col gap-2.5">
              <div className="flex items-baseline gap-2">
                <span
                  ...
                  className="text-2xl font-black font-mono text-[var(--text-primary)] leading-none tracking-tight"
                >
              ...
              {hoveredCell.models && Object.keys(hoveredCell.models).length > 0 ? (
                <div className="mt-1 border-t border-[var(--border-color)] pt-3 flex flex-col gap-2">
                  <div className="text-[11px] font-bold text-[var(--color-primary)] font-mono uppercase tracking-wider">
                    {copy("heatmap.tooltip.model_breakdown")}
                  </div>
                  <div className="flex flex-col gap-2.5 max-h-[180px] overflow-y-auto pr-2 pb-1 oai-scrollbar">
                    {Object.entries(hoveredCell.models)
                      ...
                      .map(({ name, val }) => {
                        ...
                        return (
                          <div key={name} className="flex flex-col gap-1 px-0.5">
                            <div className="flex items-center justify-between text-xs gap-3">
                              <span className="font-semibold font-mono text-[var(--text-primary)] truncate max-w-[140px]" title={name}>
                                {name}
                              </span>
                              <div className="flex items-center gap-2 shrink-0">
                                <span
                                  title={formatTokensTooltip(val)}
                                  className="font-mono text-[var(--text-primary)] font-bold text-xs"
                                >
                                  {formatTokens(val)}
                                </span>
                                <span className="text-xs text-[var(--color-primary)] min-w-[32px] text-right font-bold font-mono">
                                  {pct}%
                                </span>
                              </div>
                            </div>
                            <div className="w-full h-[4.5px] bg-white/15 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  ...
                                  backgroundImage: "linear-gradient(90deg, var(--color-primary), color-mix(in srgb, var(--color-primary) 70%, #ffffff))",
                                  boxShadow: "0 0 8px var(--color-primary), 0 0 2px rgba(255,255,255,0.6)"
                                }}
                              />
                            </div>
                          </div>
                ...
                <p className="text-xs text-[var(--text-secondary)] leading-relaxed font-normal mt-1 border-t border-[var(--border-color)] pt-2.5">
          ...
          {/* 小尾巴（默认朝下指向单元格；翻转时朝上） */}
          <div
            className={`absolute left-0 -translate-x-1/2 w-2.5 h-2.5 rotate-45 bg-[var(--bg-sidebar)] shadow-sm ${tooltipPos.flipY ? "top-[6px] border-l border-t" : "bottom-[6px] border-r border-b"} border-[var(--border-color)]`}

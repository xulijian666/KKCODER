// Vendored from upstream src/pages/SkillDetailPanel.jsx. Adaptation:
//   - cost row only renders when the backend actually provides usage.cost
//     (the built-in Rust skills backend does not port LiteLLM pricing).
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          ...
          <section className="mt-6">
            ...
            <dl className="divide-y divide-oai-gray-200/60 dark:divide-white/[0.06]">
              ...
              {/* Vendored change: the built-in Rust backend (skills_hub.rs)
                  does not compute cost (upstream's LiteLLM pricing table was
                  not ported), so the cost row only renders when a cost is
                  actually provided (e.g. browser preview via tt-dev). */}
              ...
              >
                {formatUsdCurrency(usage.cost, { currency, rate })}
              </PropRow>
              ) : null}
            </dl>
            {!hasUsage ? (
